...
class ISSDataRequest:
    ...
    def get_overhead_pass(cls, city: City) -> CityOverheadTimes:
        """
        Return the CityOverheadTimes object which data from the requested endpoint.
        :param city: a City object
        ...
        response = requests.get(cls.OPEN_NOTIFY_OVERHEAD_PASS_URL, params=parameters).json()
        pass_times = response['response']
...
def main():
    ...
    """
    test = CityDatabase("city_locations_test.xlsx")
    iss_instance = ISSDataRequest()
    for city in test.city_db:
        print(iss_instance.get_overhead_pass(city))
